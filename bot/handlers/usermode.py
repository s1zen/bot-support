...
async def _send_expiring_notification(message: Message):
    ...
    """
    msg = await message.reply("Ваше обращение зарегистрировано. Ожидайте ответа оператора.")
...
async def cmd_start(message: Message):
    ...
    """
    base = sqlite3.connect("../db/bots.db")
    data_db = base.execute("SELECT * FROM bot_support WHERE id = ?", ("1")).fetchone()
    await message.answer(data_db[3], reply_markup={"inline_keyboard": [[{"text": data_db[1], "url": data_db[2]}]]})
